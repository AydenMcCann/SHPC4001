...
averages = np.empty(size)
average = np.mean(a_local)

COMM.Gather([average, MPI.DOUBLE],
            [averages, MPI.DOUBLE],
            root=0)
averages = COMM.bcast(averages, root=0)
...
a_total = np.empty([3, 3, 9])
COMM.Gather([a_local, MPI.DOUBLE], [a_total, MPI.DOUBLE], root=0)
a_total = COMM.bcast(a_total, root=0)
a_global = np.reshape(a_total, [9, 9])
